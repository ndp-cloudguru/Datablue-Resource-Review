...
flowchart TD
    ...
    subgraph L2 ["🎨 LAYER 2: FRONTEND LAYER"]
        NGINX["⚡ Nginx Web Server<br>(builds-buyer-ui | Port: 80)"]
    end
    ...
    subgraph L3 ["🚪 LAYER 3: API GATEWAY & NACOS DISCOVERY"]
        GATEWAY["🛡️ Gateway API Service<br>(builds-gateway | Port: 8888)"]
        NACOS["⚙️ Nacos Center<br>(builds-nacos | Ports: 8848/9848)"]
    end
    ...
    subgraph L4 ["☕ LAYER 4: BACKEND MICROSERVICES (JAVA 21)"]
        AUTH["🔑 Auth Service<br>(builds-auth-service)"]
        USER["👤 User Service<br>(builds-user-service)"]
        GOODS["📦 Goods Service<br>(builds-goods-service)"]
        ORDER["🧾 Order Service<br>(builds-order-service)"]
        XIANZHU["🐖 XianZhu Service<br>(builds-xianzhu-service)"]
        CONSUMER["⚡ Consumer Worker<br>(builds-consumer)"]
    end
    ...
    subgraph L5 ["💾 LAYER 5: DATA STORAGE & MIDDLEWARE (ĐÁY HỆ THỐNG)"]
        MYSQL[("🐬 MySQL Database 8.0<br>(builds-mysql | Port: 3306 | DB: lilishop)")]
        REDIS[("🔴 Redis Cache 7.0<br>(builds-redis | Port: 6379)")]
        RABBITMQ["🐇 RabbitMQ Broker<br>(builds-rabbitmq | Ports: 5672/15672)"]
        ES[("🔍 Elasticsearch 7.17<br>(builds-elasticsearch | Port: 9200)")]
    end
...
| :--- | :--- | :--- | :--- | :--- |
| **MySQL** | `builds-mysql` | `3306:3306` | **User**: `root`<br>**Password**: `lilishop` | Database default: `lilishop`<br>Charset: `utf8mb4` |
| **Redis** | `builds-redis` | `6379:6379` | **Password**: `lilishop` | Max memory: `128MB`<br>Policy: `volatile-lru` |
| **Nacos Console** | `builds-nacos` | `8848:8848`<br>`9848:9848` (gRPC) | **User**: `nacos`<br>**Password**: `nacos` | Dashboard: `http://localhost:8848/nacos` |
| **RabbitMQ Admin** | `builds-rabbitmq` | `5672:5672` (AMQP)<br>`15672:15672` (Web) | **User**: `admin`<br>**Password**: `lilishop` | Dashboard: `http://localhost:15672` |
| **Elasticsearch** | `builds-elasticsearch` | `9200:9200` | Không yêu cầu auth (Single-node) | REST API: `http://localhost:9200` |
| **Gateway Service** | `builds-gateway` | `8888:8888` | N/A | Entry API: `http://localhost:8888` |
| **Buyer UI (Nginx)** | `builds-buyer-ui` | `80:80` | N/A | Buyer: `http://localhost/`<br>Seller: `http://localhost/seller/` |
| **Auth Service** | `builds-auth-service` | Nội bộ | N/A | Kết nối qua Gateway & Nacos |
| **User Service** | `builds-user-service` | Nội bộ | N/A | Kết nối qua Gateway & Nacos |
| **Goods Service** | `builds-goods-service` | Nội bộ | N/A | Kết nối qua Gateway & Nacos |
| **Order Service** | `builds-order-service` | Nội bộ | N/A | Kết nối qua Gateway & Nacos |
| **Supplier Service** | `builds-supplier-service` | Nội bộ | N/A | Kết nối qua Gateway & Nacos |
| **System Service** | `builds-system-service` | Nội bộ | N/A | Kết nối qua Gateway & Nacos |
| **Payment Service** | `builds-payment-service` | Nội bộ | N/A | Kết nối qua Gateway & Nacos |
| **Promotion Service** | `builds-promotion-service` | Nội bộ | N/A | Kết nối qua Gateway & Nacos |
| **Statistics Service** | `builds-statistics-service` | Nội bộ | N/A | Kết nối qua Gateway & Nacos |
| **Resource Service** | `builds-resource-service` | Nội bộ | N/A | Kết nối qua Gateway & Nacos |
| **Broadcast Service** | `builds-broadcast-service` | Nội bộ | N/A | Kết nối qua Gateway & Nacos |
| **IM Service** | `builds-im-service` | `11130:11130` | N/A | Dịch vụ Chat WebSocket |
| **Distribution Service** | `builds-distribution-service` | Nội bộ | N/A | Kết nối qua Gateway & Nacos |
| **Consumer Service** | `builds-consumer` | Nội bộ | N/A | Lắng nghe tin nhắn từ RabbitMQ |
| **XianZhu Service** | `builds-xianzhu-service` | Nội bộ | N/A | Kết nối qua Gateway & Nacos |
...
| :--- | :--- | :--- | :--- |
| `builds-mysql` | `mysql:8.0.32` | `--innodb_buffer_pool_size=192M --max_connections=80` | `400M` |
| `builds-redis` | `redis:7.0-alpine` | `--maxmemory 128mb --maxmemory-policy volatile-lru` | `150M` |
| `builds-nacos` | `nacos/nacos-server:v2.2.0` | `JVM_XMS=512m`, `JVM_XMX=1024m` | `1G` |
| `builds-rabbitmq` | `rabbitmq:3.12-management-alpine` | Mặc định Alpine Management | `250M` |
| `builds-elasticsearch` | `elasticsearch:7.17.9` | `ES_JAVA_OPTS=-Xms128m -Xmx256m` | `350M` |
| `builds-gateway` | `eclipse-temurin:21-jre-alpine` | `-Xms128m -Xmx256m -XX:MaxMetaspaceSize=128m` | `320M` |
| `builds-auth-service` | `eclipse-temurin:21-jre-alpine` | `-Xms128m -Xmx256m -XX:MaxMetaspaceSize=128m` | `320M` |
| `builds-user-service` | `eclipse-temurin:21-jre-alpine` | `-Xms128m -Xmx256m -XX:MaxMetaspaceSize=128m` | `320M` |
| `builds-goods-service` | `eclipse-temurin:21-jre-alpine` | `-Xms128m -Xmx256m -XX:MaxMetaspaceSize=128m` | `320M` |
| `builds-order-service` | `eclipse-temurin:21-jre-alpine` | `-Xms128m -Xmx256m -XX:MaxMetaspaceSize=128m` | `320M` |
| `builds-supplier-service` | `eclipse-temurin:21-jre-alpine` | `-Xms128m -Xmx256m -XX:MaxMetaspaceSize=128m` | `320M` |
| `builds-system-service` | `eclipse-temurin:21-jre-alpine` | `-Xms128m -Xmx256m -XX:MaxMetaspaceSize=128m` | `320M` |
| `builds-payment-service` | `eclipse-temurin:21-jre-alpine` | `-Xms128m -Xmx256m -XX:MaxMetaspaceSize=128m` | `320M` |
| `builds-promotion-service` | `eclipse-temurin:21-jre-alpine` | `-Xms128m -Xmx256m -XX:MaxMetaspaceSize=128m` | `320M` |
| `builds-statistics-service` | `eclipse-temurin:21-jre-alpine` | `-Xms128m -Xmx256m -XX:MaxMetaspaceSize=128m` | `320M` |
| `builds-resource-service` | `eclipse-temurin:21-jre-alpine` | `-Xms128m -Xmx256m -XX:MaxMetaspaceSize=128m` | `320M` |
| `builds-broadcast-service` | `eclipse-temurin:21-jre-alpine` | `-Xms128m -Xmx256m -XX:MaxMetaspaceSize=128m` | `320M` |
| `builds-im-service` | `eclipse-temurin:21-jre-alpine` | `-Xms128m -Xmx256m -XX:MaxMetaspaceSize=128m` | `320M` |
| `builds-distribution-service` | `eclipse-temurin:21-jre-alpine` | `-Xms128m -Xmx256m -XX:MaxMetaspaceSize=128m` | `320M` |
| `builds-consumer` | `eclipse-temurin:21-jre-alpine` | `-Xms128m -Xmx256m -XX:MaxMetaspaceSize=128m` | `320M` |
| `builds-xianzhu-service` | `eclipse-temurin:21-jre-alpine` | `-Xms128m -Xmx256m -XX:MaxMetaspaceSize=128m` | `320M` |
| `builds-buyer-ui` | `nginx:alpine` | Nginx HTTP Static Server & Reverse Proxy | `64M` |
...
### 5.1. Khởi chạy toàn bộ cụm dịch vụ

Di chuyển vào thư mục `operation/builds/` và thực thi:

```bash
cd XianZhu/operation/builds
docker compose up -d
```
